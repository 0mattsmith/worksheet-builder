...
## Ready-made worksheets for a topic

At the top of the **Templates** tab, pick a subject, then type a topic or tap one of the suggested topics. Add the pupils' year group and click **Suggest worksheets**. The AI suggests two to four complete worksheets that each take a different approach, such as a knowledge check, key vocabulary, exam practice, a reading task or a practical. Each one comes with its header, sections, questions and answers, and is laid out in a matching template.

- **More ideas** adds more suggestions that don't repeat the ones you already have. **↻ Rewrite** writes a suggestion again from scratch.
- **Preview & edit** opens the worksheet in the editor, showing it in its design. Point at any part of it to:
  - **✎ edit:** questions open in the wizard, while headings, text and checklists are edited in place;
  - **↻ rewrite a question with AI**;
  - **↑ ↓ move** it, or **✕ delete** it (**↶ Undo** brings it back);
  - on a section heading, **✨ add 2 AI questions** to that section, or **+ add your own** with the wizard.
- **Edit as text** shows the whole worksheet in the Quick build format, for bigger changes.
- **Instructions, design and colours** lets you change the look before you add the worksheet. Tick **Show answers** to check the mark scheme.
- **Create worksheet** puts it into Word with its header, design and answer data, ready for the teacher copy and answer key.

Your suggestions and edits are saved on this computer, so they're still there next time you open the add-in. Suggestions need a Claude or Gemini API key.
